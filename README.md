...
缩略图存于 `.thumbcache/`,以**图像数据的 xxhash64** 命名。哈希只覆盖图像像素流（PNG `IDAT` / JPEG `SOS` 后扫描数据）,不碰元数据区,因此往元数据里嵌入哈希后重算结果不变。

生成的哈希原子写回原图元数据（PNG `tEXt` chunk / JPEG `APP9` 段）。第二次访问只读文件头（几 KB）取出哈希,无需重算。文件移动/改名后内容不变 → 哈希不变 → 缓存命中。

支持 PNG / JPEG / WebP。WebP 暂不写回,每次按需重算。
