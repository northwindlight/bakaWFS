...
**为什么把哈希嵌在图片里。** 算一次哈希要读全文件（20MB 冷读 200ms+）。如果每次访问都重算,缓存命中也慢。把算好的哈希写进图片自身的元数据区（PNG `tEXt` 在 IHDR 后、JPEG `APP9` 在 SOI 后,都在文件头几 KB 内）,第二次直接读头部拿哈希,零全文件 I/O。哈希跟着文件走,不需要额外的索引文件或数据库。

**为什么二次重算还是一样的值。** 哈希只覆盖图像像素流（PNG `IDAT` / JPEG `SOS` 后扫描数据）,跳过元数据区。我们的哈希写在元数据区,不影响被哈希的那部分,所以嵌入后重算结果不变,可随时校验。

**为什么不用旁路索引。** 独立索引（`.hash` 文件、数据库、`.thumbcache/.index/`）多出来几千个旁路文件,移动目录要同步、删图要清理、不一致要修复。把哈希长在图片自己身上,一个文件自带身份,没有同步问题。
