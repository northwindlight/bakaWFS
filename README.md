...
**为什么不用数据库。** bakaWFS 的设计哲学是无数据库——配置是 YAML、用户是 YAML、文件列表靠 `os.ReadDir`,文件系统就是状态。为缩略图映射引入 SQLite 打破了这条线。即使引入,目录移动/重命名/删除时数据库记录要和实际文件同步,新增一种不一致的可能性。把哈希长在图片的元数据区,零额外状态,不存在同步问题。
